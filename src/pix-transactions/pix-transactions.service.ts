...
export class PixTransactionsService {
  ...
  async precheckKey(accountHolderId: string, pixKey: string, value: string) {
    ...
    throw new BadRequestException(
      'Não foi possível pré-consultar a chave Pix no provedor.',
    );
  }
  ...
  async sendPix(
    ...
    if (value < 1)
      throw new BadRequestException('O valor mínimo de pagamento é R$ 1,00.');

    const message = (dto.description ?? '').slice(0, 140);
    const identifier = uuidv4();

    // Pré-consulta é obrigatória para cumprir sua exigência
    const pre = await this.precheckKey(
      accountHolderId,
      dto.pixKey.trim(),
      dto.amount.trim(),
    );
    if (!pre?.endToEndPixKey) {
      throw new BadRequestException(
        'Pré-consulta não retornou EndToEndPixKey.',
      );
    }
    ...
    const endToEnd =
      d?.EndToEndPixKey || d?.endToEndPixKey || pre.endToEndPixKey || null;
  ...
  async createStaticQr(
    ...
    const QRCodeFormat =
      format === 'both' ? 'both' : format === 'image' ? 'image' : 'copy-paste';
