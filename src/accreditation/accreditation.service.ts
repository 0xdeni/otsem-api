...
export class AccreditationService {
  ...
  private async callBrxGet<T>(
    url: string,
    token: string,
    attempt = 1,
    maxAttempts = 3,
  ): Promise<BrxResponse<T>> {
    const correlation = randomUUID();
    const started = performance.now();
    this.logger.debug(
      `[BRX][GET][${attempt}] ${url} correlation=${correlation}`,
    );
    try {
      ...
      this.logger.log(
        `[BRX][GET][OK] ${url} status=${data.StatusCode} ms=${ms} correlation=${correlation}`,
      );
      return data;
    } catch (e: any) {
      const ms = Math.round(performance.now() - started);
      ...
      this.logger.error(
        `[BRX][GET][ERR] ${url} status=${status ?? 'N/A'} ms=${ms} attempt=${attempt}/${maxAttempts}`,
      );
  ...
  private async callBrxPost<TReq, TRes>(
    url: string,
    body: TReq,
    token: string,
    attempt = 1,
    maxAttempts = 3,
  ): Promise<BrxResponse<TRes>> {
    const correlation = randomUUID();
    const started = performance.now();
    this.logger.debug(
      `[BRX][POST][${attempt}] ${url} correlation=${correlation} bodyBytes=${Buffer.byteLength(JSON.stringify(body))}`,
    );
    try {
      const { data } = await firstValueFrom(
      ...
      if (data.StatusCode >= 400) {
        this.logger.warn(
          `[BRX][POST][WARN] ${url} status=${data.StatusCode} ms=${ms} correlation=${correlation} title=${data.Title}`,
        );
      } else {
        this.logger.log(
          `[BRX][POST][OK] ${url} status=${data.StatusCode} ms=${ms} correlation=${correlation}`,
        );
      }
      return data;
    } catch (e: any) {
      ...
      const retriable = status && status >= 500;
      this.logger.error(
        `[BRX][POST][ERR] ${url} status=${status ?? 'N/A'} ms=${ms} attempt=${attempt}/${maxAttempts}`,
      );
      if (retriable && attempt < maxAttempts) {
        await new Promise((r) => setTimeout(r, attempt * 400));
        return this.callBrxPost<TReq, TRes>(
          url,
          body,
          token,
          attempt + 1,
  ...
  // -------- PF --------
  async accreditPerson(
    dto: AccreditationPersonDto,
    ...
    this.logger.log(
      `Credenciar PF identifier=${dto.identifier} cpf=${this.mask(dto.cpf)}`,
    );

    try {
      const data = await this.callBrxPost<
        typeof body,
        AccreditationPFResponseData
      >(url, body, token);
      ...
      if (customer) {
        ...
        this.logger.log(
          `Customer atualizado id=${customer.id} externalAccredId=${d.AccreditationId}`,
        );
      } else {
        this.logger.warn(
          `Customer local não encontrado cpf=${this.mask(d.Person.Cpf)}`,
        );
      }
      ...
      this.logger.error(
        `Credenciamento PF falhou identifier=${dto.identifier} cpf=${this.mask(dto.cpf)} msg=${msg}`,
      );
  ...
  // -------- PJ --------
  async accreditCompany(
    dto: AccreditationCompanyDto,
  ): Promise<AccreditationResult> {
    const token = await this.brxAuth.getAccessToken();
    ...
    this.logger.log(
      `Credenciar PJ identifier=${dto.identifier} cnpj=${this.mask(dto.cnpj)}`,
    );
    ...
    try {
      ...
      if (customer) {
        ...
        this.logger.warn(
          `Customer local não encontrado cnpj=${this.mask(d.Company.Cnpj)}`,
        );
      }
      ...
      this.logger.error(
        `Credenciamento PJ falhou identifier=${dto.identifier} cnpj=${this.mask(dto.cnpj)} msg=${msg}`,
      );
      throw new BadRequestException(msg);
    }
  }
  ...
  async getAccreditationById(accreditationId: string) {
    ...
    try {
      const data = await this.callBrxGet<
        AccreditationPFResponseData | AccreditationPJResponseData
      >(url, token);
      if (data.StatusCode >= 400)
        throw new BadRequestException(data.Extensions?.Message || data.Title);
  ...
  async getAccreditationByCpfDirect(cpf: string) {
    ...
    try {
      const data = await this.callBrxGet<AccreditationPFResponseData>(
        url,
        token,
      );
      if (data.StatusCode >= 400)
  ...
  async getAccreditationByCnpjDirect(cnpj: string) {
    ...
    try {
      const data = await this.callBrxGet<AccreditationPJResponseData>(
        url,
        token,
      );
      if (data.StatusCode >= 400)
  ...
  async findCustomerByCpf(cpf: string) {
    ...
    if (!customer)
      throw new BadRequestException('Customer não encontrado (CPF).');
    if (!customer.externalAccredId)
      throw new BadRequestException('Customer sem credenciamento.');
    this.logger.debug(
      `Lookup local + BRX CPF=${this.mask(clean)} accredId=${customer.externalAccredId}`,
    );
  ...
  async findCustomerByCnpj(cnpj: string) {
    ...
    if (!customer)
      throw new BadRequestException('Customer não encontrado (CNPJ).');
    if (!customer.externalAccredId)
      throw new BadRequestException('Customer sem credenciamento.');
    this.logger.debug(
      `Lookup local + BRX CNPJ=${this.mask(clean)} accredId=${customer.externalAccredId}`,
    );
    return this.getAccreditationById(customer.externalAccredId);
  }
}
