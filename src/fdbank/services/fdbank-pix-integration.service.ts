...
export class FdbankPixIntegrationService {
    ...
    async sendPix(
        ...
        try {
            ...
            const pixKeyType = pixKeyTypeMap[dto.tipoChave] || 'cpf';

            // Step 1: Resolve the destination PIX key via DICT to get endToEndId
            this.logger.log(`Resolving PIX key via DICT: ${dto.chaveDestino} (${pixKeyType})`);
            const dictResult = await this.pixKeyService.resolveDict({
                pixKey: dto.chaveDestino,
                pixKeyType,
                value: dto.valor,
            });
            const resolved = dictResult.result || dictResult;
            this.logger.log(`DICT resolved: endToEndId=${resolved.endToEndId}, name=${resolved.name}`);

            if (!resolved.endToEndId) {
                throw new BadRequestException('Could not resolve PIX key - no endToEndId returned');
            }

            // Step 2: Execute the transfer with the resolved endToEndId
            const payload = {
                pixKey: dto.chaveDestino,
                pixKeyType,
                value: dto.valor,
                endToEndId: resolved.endToEndId,
