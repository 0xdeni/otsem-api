...
import type {
  BrxPrecheckRaw,
  PrecheckKeyResponseDto,
} from './dtos/precheck-key.dto';
...
export class PixService {
  ...
  async precheckKey(
    ...
    try {
      ...
      const ax = err as AxiosError<{ message?: string }>;
      const msg =
        ax.response?.data?.message ?? ax.message ?? 'Erro na pré-consulta BRX';
      throw new HttpException(msg, ax.response?.status ?? 502);
    }
  }
  ...
  async deleteKey(
    ...
    try {
      ...
      return {
        ok: true,
        message:
          resp.data?.Extensions?.Message ?? 'Chave Pix removida com sucesso',
      };
    } catch (err) {
      const ax = err as AxiosError<{ message?: string }>;
      console.error('🔴 [BRX DELETE] Erro:', ax.response?.data || ax.message);
      const msg =
        ax.response?.data?.message ?? ax.message ?? 'Erro ao remover chave BRX';
      throw new HttpException(msg, ax.response?.status ?? 502);
    }
  }
}
