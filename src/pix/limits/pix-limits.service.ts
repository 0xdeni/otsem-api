...
import { UpdatePixLimitDto, BrxPixLimitResponse, PixLimitResult } from './dto/update-pix-limit.dto';
...
export class PixLimitsService {
     ...
     */
    async setPixLimits(accountHolderId: string, productId: number, dto: UpdatePixLimitDto): Promise<PixLimitResult> {
        const token = await this.brxAuth.getAccessToken();

        const url =
            `${this.baseUrl}/product/parameters/account-holders/${accountHolderId}/products/${productId}/limit-pix`;
